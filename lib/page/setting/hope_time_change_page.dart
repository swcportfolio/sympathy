...
class HopeTimeChangePage extends StatefulWidget {

  final Authorization auth;
  final VoidCallback callback;
  HopeTimeChangePage(this.auth, this.callback);

  @override
  _HopeTimeChangePageState createState() => _HopeTimeChangePageState();
}

class _HopeTimeChangePageState extends State<HopeTimeChangePage> {
  final String title = '상담시간 설정';
  int timeHour = 9; // 시간 설정-최소 9시부터
  SetHopeTime setHopeTime ;  //hopeTime 객체 생성
  final headText  = '* 상담사가 지정되면 변경할 수 없습니다.(상담사에게 문의 바랍니다)';
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(headText,textScaleFactor: 1.1, style: TextStyle(fontWeight: FontWeight.bold)),
            Row(
              children: [
            ...
            HopeTimeButton(btnName: '설정완료', setHopeTime:setHopeTime, auth: widget.auth, context: context, callback: ()=> widget.callback(), division: 'setting',)
          ],
        ),
      ),
    );
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(headText == '생년월일'? headText: identifier.toString()+ '순위 '+headText,
              style: TextStyle(color:headText =='(필수 *)'?Colors.red:Colors.black, fontSize: 14, fontWeight: FontWeight.bold)),
        ),
            ...
            child: Padding(padding: const EdgeInsets.all(8.0),
              child: Row(
                children: [
                  ...
                  Text(hint, style: TextStyle(color: Colors.grey, fontSize: 12)),
              ...
              child: Padding(padding: const EdgeInsets.all(8.0),
                child: Row(
                  children: [
                    ...
                    Text(hint, style: TextStyle(color: Colors.grey, fontFamily: 'Opensans', fontSize: 12)),
                  ],
  ...
  Future _showIntegerDialog(int identifier) async {
    await showDialog<int>(
      ...
      builder: (BuildContext context) {
        return NumberPickerDialog.integer(selectedTextStyle: TextStyle(color: mainColor,fontSize: 25,fontWeight: FontWeight.bold),
      ...
      if(value != null){
        if(value is int){
          setState(() {
            ...
            if(identifier == setHopeTime.identifier_1)  setHopeTime.hourTime1 = value.toString()+'시';
            else if(identifier == setHopeTime.identifier_2)  setHopeTime.hourTime2 = value.toString()+'시';
            else if(identifier == setHopeTime.identifier_3)  setHopeTime.hourTime3 = value.toString()+'시';
  ...
  // void _setHopeTime() {
  //   checkEmptyHopeTimeDate(userDetails.hopeTime1);
  //   //  SetHopeTime setHopeTime = SetHopeTime(dateTime1, dateTime2, dateTime3, hourTime1, hourTime2, hourTime3)
  //
  // }
  // String checkEmptyHopeTimeDate(String value){
  //   if(value == '-')
  //   {
  //     return '';
  //   }else{
  //     String target = ' ';
  //     int targetNum = value.indexOf(target);
  //     String result = value.substring(0,targetNum);
  //     print('====> hopeTimeDate parsing value : '+ result);
  //     return result;
  //   }
  // }
  //
  // String checkEmptyHopeTimeHour(String value){
  //   if(value == '-')
  //   {
  //     return '';
  //   }else{
  //     String target = ' ';
  //     int targetNum = value.indexOf(target);
  //     String result = value.substring(targetNum,value.length);
  //     print('====> hopeTimeHour parsing value : '+ result);
  //     return result;
  //   }
  // }
}
